programa
{
	...
	funcao inicio(){
						...
						para(inteiro i = 1; i <= 6; i++){
							para(inteiro j = 1; j <= 6; j++){
								se(pedidos[j] != "0" e pedidosJaInseridos[j] != itemsInfo[j][1]){
									...
									inteiro qntInt = tp.real_para_inteiro(qntConvertido)
									msg = ("[" + i + "] " + itemsInfo[j][1] + " R$" + calc + " (" + qntInt + "x)")
									txLft(msg)
		...
		encerrar = falso
		back2Default()
		feedbackMsg1 = "Fase do pagamento."
		enquanto(encerrar == falso){
			limpa()
			txLine()
		...
		escolha(IDpedido){
			caso 1:
			caso 2:
				inteiro cardPassword
				escreva("Insira a senha do cartão: ")
				leia(cardPassword)
				...
				txMid(msg)
				txMid(" ")
				txMid("Cartão Master Card")
				txMid("Joinville - SC    ")
				txMid("Lanchonete - CNPJ 11.444.777/0001-61")
				txMid(" ")
				txMid(" ")
				real valorCompra = 0.0
				maxQnt = 0.0
				para(inteiro i = 1; i <= 6; i++){
					...
					valorCompra += qntConvert * precoConvert
					valorCompra *= 0.85
					se(qntDesconto == verdadeiro){
						valorCompra *= 0.90
					}
				}
				msg = "R$: " + valorCompra
				txMid("Descontos aplicados.")
				txMid("Valor final:")
				txMid(msg)
				codigo = u.sorteia(20000000, 90000000)
				convertCodigo = tp.inteiro_para_cadeia(codigo, 10)
				txMid(convertCodigo)
				txMid(" ")
				txMid(" ")
				txRght("Volte sempre!")
			...
			caso 3:
				maxQnt = 0.0
				real valorCompra2 = 0.0
				para(inteiro i = 1; i <= 6; i++){
				real qntConvert = tp.cadeia_para_real(pedidos[i])
				maxQnt += qntConvert
				real precoConvert = tp.cadeia_para_real(itemsInfo[i][2])
				valorCompra2 += qntConvert * precoConvert
				se(qntDesconto == verdadeiro){
					valorCompra2 *= 0.90
				}
				}
				...
				txMid("Pagamento - Lanchonete")
				txMid(" ")
				txMid("Valor final com descontos:")
				txMid(msg)
				txMid(" ")
				txMid("Leia o código de barras")
				txMid("para prosseguir com o")
				txMid("pagamento.")
				txMid(" ")
				txMid("┃▏║│║║┃┃▏║│║┃┃▏║│║")
				txMid("┃▏║│║║┃┃▏║│║┃┃▏║│║")
 ...
 * 
 * @POSICAO-CURSOR = 7467; 
 * @DOBRAMENTO-CODIGO = [48, 54, 76, 91, 106, 114];
 * @PONTOS-DE-PARADA = ;
